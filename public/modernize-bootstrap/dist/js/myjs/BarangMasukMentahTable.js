...
function refreshTombolAction() {
  $("#tambahDataModalButton").on("click", function () {
    $.ajax({
      url: baseUrl + "/barangmasukmentahapi/getNewIdTransaksi",
      type: "GET",
      dataType: "json",
  ...
  $(".btn-edit")
    .off("click")
    .on("click", function () {
      var rowIndex = $("#barangMasukMentahTable")
        .DataTable()
        .row($(this).closest("tr"))
        .index();
      var row = $("#barangMasukMentahTable").DataTable().row(rowIndex).data();
      ...
      var optionSupplier = $("#updateForm")
        .find("#idSupplier option")
        .filter(function () {
          var text = $(this).text().trim();
          namaSupplier = text.substring(text.indexOf("-") + 1).trim();
          return namaSupplier == row.namaSupplier;
        });
      optionSupplier.prop("selected", true);
      $("#updateForm").find("#idBarangMentah").val(row.idBarangMentah);
...
$("#inputForm").submit(function (e) {
  ...
  $.ajax({
    url: baseUrl + "/barangmasukmentahapi/inputData",
    type: "POST",
...
$("#updateForm").submit(function (e) {
  ...
  $.ajax({
    url: baseUrl + "/barangmasukmentahapi/updateData",
    type: "PUT",
    dataType: "json",
    contentType: "application/json",
    data: JSON.stringify({
...
$("#deleteForm").submit(function (e) {
  ...
  $.ajax({
    url: baseUrl + "/barangmasukmentahapi/deleteData",
    type: "PUT",
    dataType: "json",
    contentType: "application/json",
    data: JSON.stringify({
    ...
    success: function (response) {
      ...
      // Refresh fungsi tombol update + hapus
      refreshTombolAction();
