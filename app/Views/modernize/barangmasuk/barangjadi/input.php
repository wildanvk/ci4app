...
<div class="card bg-light-info shadow-none position-relative overflow-hidden">
    <div class="card-body px-4 py-3">
        <div class="row align-items-center">
            <div class="col-9">
                <h4 class="fw-semibold mb-8">Input Transaksi Masuk Barang Mentah</h4>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a class="text-muted">Master</a></li>
                        <li class="breadcrumb-item" aria-current="page"><a href="/stokbarangmentah">Transaksi Masuk Barang Mentah</a></li>
                        <li class="breadcrumb-item" aria-current="page">Input Transaksi Masuk Barang Mentah</li>
                    </ol>
                </nav>
            </div>
            <div class="col-2">
...
<div class="card w-100 position-relative overflow-hidden">
    <div class="px-4 py-3 border-bottom d-flex justify-content-between align-items-center bg-primary">
        <h5 class="card-title fw-semibold mb-0 lh-sm text-white">Input Transaksi Masuk Barang Mentah</h5>
    </div>
    <form action="/barangmasukmentah/store" method="post">
        <?= csrf_field() ?>
                ...
                <div class="col-sm-6">
                    <select class="form-select" name="idBarangMentah" id="idBarangMentah">
                        ...
                        <?php foreach ($barangmentah as $key) { ?>
                            <option value="<?= $key['idBarangMentah'] ?>" <?= old('idBarangMentah') == $key['idBarangMentah'] ? 'selected' : '' ?>><?= $key['idBarangMentah'] . ' - ' . $key['namaBarangMentah'] ?></option>
                        <?php } ?>
                    </select>
                </div>
            </div>
                ...
                <div class="col-sm-6">
                    <select class="form-select" name="idSupplier" id="idSupplier">
                        ...
                        <?php foreach ($supplier as $key) { ?>
                            <option value="<?= $key['idSupplier'] ?>" <?= old('idSupplier') == $key['idSupplier'] ? 'selected' : '' ?>><?= $key['idSupplier'] . ' - ' . $key['namaSupplier'] ?></option>
                        <?php } ?>
                    </select>
                </div>
            </div>
